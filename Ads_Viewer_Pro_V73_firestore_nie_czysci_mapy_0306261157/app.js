var APP_VERSION_NUMBER = "V73";
var APP_VERSION_STAMP = "0306261157";
...
var FIRESTORE_STATE_SECTIONS = ["theme", "watchlist", "alertSettings", "alertFired"];
var FIRESTORE_RUNTIME_REPAIR_KEY = "adsb-v73-firestore-runtime-repair-v1";
...
function repairRuntimeSettingsAfterV72FirestoreSync() {
  if (storageGet(FIRESTORE_RUNTIME_REPAIR_KEY, "") === APP_VERSION_STAMP) return;

  const config = loadFirestoreConfig();
  if (!firestoreConfigComplete(config)) {
    storageSet(FIRESTORE_RUNTIME_REPAIR_KEY, APP_VERSION_STAMP);
    return;
  }

  const savedSourceName = storageGet(DATA_SOURCE_STORAGE_KEY, DEFAULT_DATA_SOURCE);
  const savedSource = API_SOURCES[savedSourceName];
  const savedApiKey = storageGet(API_KEY_STORAGE_KEY, "").trim();

  if (!savedSource || (savedSource.requiresKey && !savedApiKey)) {
    storageSet(DATA_SOURCE_STORAGE_KEY, DEFAULT_DATA_SOURCE);
    storageSet(API_BASE_STORAGE_KEY, apiSourceByName(DEFAULT_DATA_SOURCE).apiBase);
  }
...
repairRuntimeSettingsAfterV72FirestoreSync();
loadAircraftFilters();
