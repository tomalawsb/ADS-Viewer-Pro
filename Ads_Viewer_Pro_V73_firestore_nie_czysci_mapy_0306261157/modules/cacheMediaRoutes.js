...
async function loadAircraft() {
  ...
  const hadAircraftBeforeFetch = Array.isArray(lastAircraftCache) && lastAircraftCache.length > 0;
  try {
    ...
    try {
      ...
      const keepText = hadAircraftBeforeFetch ? " Poprzednie samoloty zostają na mapie." : "";
      setAircraftStatus(`Nie udało się pobrać danych. Ostatni błąd: ${details}.${keepText}`);
      showToast(`Błąd pobierania: ${details}${keepText}`, 6200);
    }
  } finally {
    finishBusy();
  }
